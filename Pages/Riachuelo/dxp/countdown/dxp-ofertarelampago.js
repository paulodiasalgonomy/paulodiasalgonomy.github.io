...
function initializeCarousel() {
    ...
    function buildProducts(products) {
        ...
        for (var i = 0, len = products.length; i < len; i++) {
            ...
            html += `<a class="item click-trigger click" href="` + products[i].clickURL + `" title="Ir para ` + products[i].name + `" onClick="javascript:pushClickDatalayer('` + dlPriceCents + `','` + dlIsMkt + `','` + dlName + `','` + products[i].id + `','` + dlList + `','` + String(i + 1) + `','` + dlBrand + `','` + dlCategory + `','` + dlSalesPrice + `');">
        ...
        html += "</div><a href='" + dxpButtonLink + "'title='" + dxpButtonText + "' onClick='javascript:pushClickButtonListDatalayer();' class='click-trigger click' id='dxp-button-link'><button class='click-trigger'  id='dxp-button'>" + dxpButtonText + "</button></a></div>";
